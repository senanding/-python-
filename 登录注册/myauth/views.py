...
@login_required(login_url='myauth:login')
def bind(request):#用户绑定
    if request.method == "POST":
        return redirect(request,'myauth:userCenter.html')
    else:
        return render(request, 'myauth/bind.html')
...
def indexpage(request):#用户中心
    return render(request,'myauth/indexpage.html')